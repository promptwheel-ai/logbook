...
4. `init` writes neutral `--by MODEL` attribution and creates the
   CLAUDE.md `@AGENTS.md` bridge on fresh repos; the exact old generated
   block migrates, user-edited blocks are never touched.
5. `DEFAULT_MAX` is the single source for the commit window — the ledger
   cache gate in `loadEvents` depends on it.
6. Never turn "unmeasurable" into clean: a failed diff scan sets
